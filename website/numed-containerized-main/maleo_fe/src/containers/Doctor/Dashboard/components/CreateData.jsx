...
const MedicalRecordDetail = (history, info) => {
  history.push({
      ...
      for (let i = 0; i < dataAPI.length; i += 1) {
        if (dataAPI.length === 0) {
          ...
          data.push({
            id: dataAPI[i].id,
            number: `${i + 1}`,
            mr_id: dataAPI[i].mr_id_patient,
            patient_name: dataAPI[i].fullname,
            patient_status: dataAPI[i].sex,
            dob: dataAPI[i].dob,
            status: [
              // eslint-disable-next-line no-loop-func
              // eslint-disable-next-line max-len
              (<Button color="success" onClick={() => MedicalRecordDetail(history, dataAPI[i])}>Detail</Button>),
            ],
          });
        }
      }
