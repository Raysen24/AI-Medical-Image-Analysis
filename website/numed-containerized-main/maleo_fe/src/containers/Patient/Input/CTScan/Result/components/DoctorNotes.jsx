      ...
      <Card>
        <CardBody>
          <div className="card__title">
            <h5 className="bold-text">{t('patient.input.ctscan.notes')}</h5>
          </div>
          <form
            className="form"
            onSubmit={(e) => {
              ...
              axios.patch(`${CTSCAN_MEDICALRECORD}${params.id}/`, updatedData, options)
                .then(() => {
                  history.push('/patient-data/input/ct-scan');
                })
                .catch((err) => {
                  console.log(err);
                });
