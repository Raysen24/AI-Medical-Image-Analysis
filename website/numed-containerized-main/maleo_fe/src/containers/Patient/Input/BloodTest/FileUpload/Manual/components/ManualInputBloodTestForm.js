...
const ManualInputBloodTestForm = ({ handleSubmit }) => {
  ...
  const hospitalList = loading && userLogin.hospital_list
    ? userLogin.hospital_list.map((e) => ({
      ...
      <Card>
        <CardBody className="dashboard__booking-card">
          <div className="card__title">
            <h5 className="bold-text">{t('patient.input.blood.cxr_upload')}</h5>
            <h5 className="subhead">Input your blood test data</h5>
          </div>
          <form className="form" onSubmit={handleSubmit}>
            <div className="form__half">
              {
              loading && userLogin.role === 'Patient'
                ? (
                  <>
                    <input
                      name="patient_id"
