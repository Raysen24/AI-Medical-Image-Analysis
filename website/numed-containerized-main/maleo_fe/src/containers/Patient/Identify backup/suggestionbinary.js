...
const SuggestionBinary = () => {
  ...
  const createCommentFile = async () => {
    try {
      ...
      const response = await axios.post('http://127.0.0.1:9090/BINARY/create-comment-file', formData, {
  ...
  const createRating = async () => {
    try {
      ...
      const response = await axios.post('http://127.0.0.1:9090/BINARY/create-rating', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
  ...
  const editCommentFile = async () => {
    try {
      ...
      const response = await axios.post('http://127.0.0.1:9090/BINARY/edit-comment-file', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
