...
async def predict():
    predicted_label = patient_predict(BINARY_PATIENT_PATH, BINARY_MODEL_PATHS, BINARY_RESULTS_PATH)
...
def predict():
    try:
        final_prediction = body_part_detect(MULTICLASS_PATIENT_PATH, MULTICLASS_MODEL_PATHS, META_MODEL_FILE, TEMPLATE_PATHS, DF_TEST_FILE)
        return {"Predicted Label": final_prediction}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def MULlist_images_and_content():
    base_path = Path(r"C:\Users\richie\Documents\data\website\website\IDENTIFIER1\Multiclass\results")
    response = []
...
async def BINlist_images_and_content():
    base_path = Path(r"C:\Users\richie\Documents\data\website\website\IDENTIFIER1\Binary\results")
    response = []
...
binary_path = Path(r"C:\Users\richie\Documents\data\website\website\IDENTIFIER1\Binary\results")
multiclass_path = Path(r"C:\Users\richie\Documents\data\website\website\IDENTIFIER1\Multiclass\results")
