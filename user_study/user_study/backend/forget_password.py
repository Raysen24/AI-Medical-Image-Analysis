...
def create_reset_token(doctor_id: int, expires_delta: timedelta = timedelta(hours=1)):
    encode = {"id": doctor_id}
    if expires_delta:
...
async def forgot_password(request: PasswordResetRequest, db: Session = Depends(get_db)):
    user = db.query(Doctor).filter(Doctor.doctor_email == request.email).first()
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="User not found")

    reset_token = create_reset_token(user.doctor_id)
    reset_link = f"http://localhost:5173/emailverification?token={reset_token}"

    send_reset_email(user.doctor_email, reset_link)
